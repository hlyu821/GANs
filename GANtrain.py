...
def GenerateData(path):
    ...
    for i in range(num):
        for idx in range(c_dim):
            ...
            resLabel.append(idx) #在这段代码中，resData 每次迭代生成一个样本，每个样本包含 mb_size 个像素块的平均值。这个过程重复 num 次，对于每个类别 c_dim 生成样本#
...
path = r'model/model_10.pth'
data,label = GenerateData(path)  #生成器生成新数据
np.save('data_test/dataV1.npy',data)
np.save('data_test/labelV1.npy',label)
data = np.load('data_test/labelV1.npy')
print(label.shape)
print("Data Content (first 2 elements):", label[:1])